...
BINARY = (
    ...
    / "cpu"
    / "programs"
    / "ddr_dma_control_ddr_heavy_pkt500_x86"
)
RUN_LABEL = "smartnic_control_ddr_heavy_ppe_none_pkt500"


def configure_options(options):
    options.binary = str(BINARY)
    options.num_packets = 500
    options.sim_cycles = max(options.sim_cycles, 10_000_000)
    options.abs_max_tick = max(options.abs_max_tick, 10_000_000_000)
...
print("[smartnic_control_ddr_heavy_ppe_none_pkt500] PASS")
